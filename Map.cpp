	...
	bool visitNode(TCODBsp *node, void *userData) {
		if ( node->isLeaf() ) {    
			...
			if ( roomNum != 0 ) {
				...
				roomNum++;
				if ( dice < 25 ) { //A roll of 50 on a d100 allows for a relatively 'even' distribution of room types.
					map.specialRoom(roomNum == 0, x, y, x+w-1, y+h-1, withActors);
				}
				// dig a corridor from last room
				map.dig(lastx,lasty,x+w/2,lasty);
				map.dig(x+w/2,lasty,x+w/2,y+h/2);
...
void Map::specialRoom(bool first, int x1, int y1, int x2, int y2, bool withActors) {
	dig (x1,y1,x2,y2);
	if (!withActors) {
		return;
	}
	Actor *redDot=new Actor(x1,y1,'.',"red 'special room' indicator dot",TCODColor::red);
	redDot->x=(x1+x2)/2;
	redDot->y=(y1+y2)/2;
	engine.actors.push(redDot);
}
